...
-- Update the status check constraint to include 'schedule'
ALTER TABLE leads 
DROP CONSTRAINT IF EXISTS leads_status_check;

ALTER TABLE leads
ADD CONSTRAINT leads_status_check 
CHECK (status IN ('unassigned', 'approved', 'declined', 'scheduled', 'schedule'));
...
WHERE status IN ('schedule', 'scheduled');
